...
export default function Toast({ message, onUndo, onDone }) {
  ...
  return (
    <div style={{ position: "absolute", bottom: 62, left: 16, right: 16, zIndex: 100, animation: "toast-in 0.2s ease" }}>
      ...
      <div style={{ background: T.deepPurple, borderRadius: r.lg, overflow: "hidden", boxShadow: "0 8px 24px rgba(44,31,69,0.28)" }}>
        <div style={{ padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <span style={{ fontFamily: font, fontSize: 13, color: T.white, flex: 1 }}>{message}</span>
          <button onClick={onUndo} style={{ background: "none", border: "none", fontFamily: font, fontSize: 13, fontWeight: 600, color: T.lilac, cursor: "pointer", padding: 0, flexShrink: 0 }}>Undo</button>
        </div>
        <div style={{ height: 2, background: "rgba(255,255,255,0.15)", position: "relative" }}>
          <div style={{ position: "absolute", left: 0, top: 0, height: "100%", width: `${progress}%`, background: T.lilac, transition: "width 0.05s linear" }} />
        </div>
      </div>
    </div>
  );
