...
export function StateScreen({ step, totalSteps, onBack, onNext, initial }) {
  ...
  const [localModalState, setLocalModalState] = useState(null)

  const selected = demo.inDemo ? demo.stateCode : localSelected
  const dropdownOpen = demo.inDemo ? demo.dropdownOpen : localDropdownOpen
  const setDropdownOpen = demo.inDemo ? demo.setDropdownOpen : setLocalDropdownOpen
  const modalState = demo.inDemo ? (demo.modalOpen ? demo.stateCode : null) : localModalState

  const handleSelect = (code) => {
    if (demo.inDemo) return
    setLocalSelected(code)
    setLocalDropdownOpen(false)
    setLocalModalState(code)
  }

  const handleModalContinue = () => {
    if (demo.inDemo) return
    onNext && onNext(localModalState)
    setLocalModalState(null)
  }
