...
export function BasicInfo({ onBack, onNext }) {
  ...
  const [localCareType, setLocalCareType] = useState(null)
  const [localPayer, setLocalPayer] = useState(null)
  const [localMeetMode, setLocalMeetMode] = useState(null)
  const [localSheetOpen, setLocalSheetOpen] = useState(false)
  const [localPayerOpen, setLocalPayerOpen] = useState(false)

  const zip = demo.inDemo ? demo.zip : localZip
  const setZip = demo.inDemo ? demo.setZip : setLocalZip
  const dob = demo.inDemo ? demo.dob : localDob
  const setDob = demo.inDemo ? demo.setDob : setLocalDob
  const careType = demo.inDemo ? demo.careType : localCareType
  const setCareType = demo.inDemo ? demo.setCareType : setLocalCareType
  const payer = demo.inDemo ? demo.payer : localPayer
  const setPayer = demo.inDemo ? demo.setPayer : setLocalPayer
  const meetMode = demo.inDemo ? demo.meetMode : localMeetMode
  const setMeetMode = demo.inDemo ? demo.setMeetMode : setLocalMeetMode
  const sheetOpen = demo.inDemo ? demo.careSheetOpen : localSheetOpen
  const setSheetOpen = demo.inDemo ? demo.setCareSheetOpen : setLocalSheetOpen
  const payerOpen = demo.inDemo ? demo.payerDropdownOpen : localPayerOpen
  const setPayerOpen = demo.inDemo ? demo.setPayerDropdownOpen : setLocalPayerOpen
      ...
      {showPayer && (
        <Field label="How would you like to pay?" style={{ marginTop: 18 }}>
          <SelectButton
            ...
            demoAttr="payer-select"
          />
          {payer && payer !== 'Self-pay (out of pocket)' && (
            <div data-demo="coverage-card">
...
function TextInput({ value, onChange, placeholder, inputMode, maxLength, demoAttr }) {
  return (
    <input
      value={value}
...
function SelectButton({ value, placeholder, onClick, demoAttr }) {
  return (
    <button
      onClick={onClick}
      data-demo={demoAttr}
