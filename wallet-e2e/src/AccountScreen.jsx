  ...
  return (
    <div style={{ flex: 1, overflowY: "auto", padding: "20px 20px 12px", display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        <div style={{ width: 56, height: 56, borderRadius: "50%", background: T.lilacLight, border: `2px solid ${T.lilac}`, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: font, fontSize: 18, fontWeight: 600, color: T.deepPurple, flexShrink: 0 }}>SW</div>
        <div>
          <div style={{ fontFamily: font, fontSize: 18, fontWeight: 600, color: T.deepPurple }}>Sandra Williams</div>
      ...
      {alerts.length > 0 ? (
        <div>
          ...
          <div style={{ marginTop: 10, background: T.white, border: `1px solid ${T.warmLight}`, borderRadius: r.lg + 2, overflow: "hidden" }}>
            {alerts.map((a, i) => (
              <div key={a.id} style={{ padding: "14px 16px", borderBottom: i < alerts.length - 1 ? `0.5px solid ${T.warmLight}` : "none", display: "flex", gap: 12, alignItems: "center" }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontFamily: font, fontSize: 13, fontWeight: 600, color: T.deepPurple }}>{a.title}</div>
      ...
      <Card>
        ...
        <div style={{ marginTop: 10 }}>
          {[{ icon: "fa-cake-candles", k: "Date of birth", v: "07/22/1985" }, { icon: "fa-phone", k: "Phone", v: "(773) 555-0182" }, { icon: "fa-location-dot", k: "Address", v: "5412 N Kedzie Ave, Chicago IL" }].map(({ icon, k, v }, i, a) => (
            <div key={k} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontFamily: font, fontSize: 13, paddingBottom: 10, marginBottom: 10, borderBottom: i < a.length - 1 ? `0.5px solid ${T.warmLight}` : "none" }}>
              ...
              <span style={{ color: T.deepPurple }}>{v}</span>
            </div>
          ))}
        </div>
      </Card>
        ...
        <div style={{ marginTop: 10 }}>
          {[{ icon: "fa-id-card", k: "Plan", v: "BCBS PPO Blue Choice" }, { icon: "fa-hashtag", k: "Member ID", v: "BCBS-772-441-09" }, { icon: "fa-circle-check", k: "Status", v: "Active ✓" }].map(({ icon, k, v }, i, a) => (
            <div key={k} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontFamily: font, fontSize: 13, paddingBottom: 10, marginBottom: 10, borderBottom: i < a.length - 1 ? `0.5px solid ${T.warmLight}` : "none" }}>
              ...
              <span style={{ color: v.includes("✓") ? T.green : T.deepPurple, fontWeight: v.includes("✓") ? 600 : 400 }}>{v}</span>
            </div>
          ))}
        </div>
      ...
      <div style={{ background: T.white, border: `1px solid ${T.warmLight}`, borderRadius: r.lg + 2, overflow: "hidden" }}>
        {menuItems.map(({ label, icon, danger }, i, arr) => (
          <HoverRow key={label} style={{ padding: "13px 16px", fontFamily: font, fontSize: 14, color: danger ? T.vibrantPurple : T.deepPurple, fontWeight: danger ? 600 : 400, borderBottom: i < arr.length - 1 ? `0.5px solid ${T.warmLight}` : "none", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <Icon name={icon} weight="thin" size={14} style={{ color: danger ? T.vibrantPurple : T.lilac, width: 16 }} />{label}
            </span>
            {!danger && <Icon name="fa-chevron-right" weight="thin" size={11} style={{ color: T.warmLight }} />}
          </HoverRow>
        ))}
      </div>
    </div>
